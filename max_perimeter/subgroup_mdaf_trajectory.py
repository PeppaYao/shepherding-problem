...
def run_animation(all_sheep, sheep_dict, herd):
    ...
    plt.scatter(X[:, 0], 600 - X[:, 1], label="group A", zorder=3)
    plt.scatter(Y[:, 0], 600 - Y[:, 1], label="group B", zorder=3)
    ...
    plt.legend()
    plt.show()
    fig.savefig('E:\\我的坚果云\\latex\\doubleDistSum\\pics\\subgroup_mdaf_trajectory.pdf', dpi=600, format='pdf')
    ## 输出距离
