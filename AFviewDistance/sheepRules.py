...
def sheeps_move(shepherd_farthest, all_sheep,
    ...
    for i in range(number_of_sheep):
        ...
        centroid_to_sheep_distance_angle = np.linalg.norm(per_sheep - shepherd_farthest)
        # 获取到局部中心点和靠太近的排斥力
        ...
        if centroid_to_sheep_distance_angle <= sheep_view_distance:
            repulsion_force_1 = (per_sheep - shepherd_farthest) / centroid_to_sheep_distance_angle
        ...
        if np.linalg.norm(current_vector) != 0:
            ...
            current_unit_vector = np.array([np.random.randint(-1, 1), np.random.randint(-1, 1)])
